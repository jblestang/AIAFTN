//! Tests de robustesse avec des datasets invalides NMEA
...
fn test_reject_invalid_nmea_dataset() {
...
#[test]
#[ignore]
fn test_invalid_nmea_error_categories() {
    let dataset_path = "tests/samples/nmea/nmea_invalid_dataset.txt";
    ...
    for line in &lines {
        match NmeaParser::parse_message(line.trim()) {
            Ok(_) => {
                parsed_count += 1;
...
#[test]
#[ignore]
fn test_invalid_nmea_performance() {
    let dataset_path = "tests/samples/nmea/nmea_invalid_dataset.txt";
    ...
    for line in &lines {
        if NmeaParser::parse_message(line.trim()).is_err() {
            rejected_count += 1;
        }
    }
...
/// Test spécifique pour les checksums invalides
#[test]
#[ignore]
fn test_invalid_checksum_rejection() {
    ...
    let lines: Vec<&str> = content.lines()
        .filter(|l| !l.trim().is_empty() && l.contains("*XX"))
        .collect();
    
    let mut checksum_error_count = 0;
    
    for line in &lines {
        if let Err(e) = NmeaParser::parse_message(line.trim()) {
            let error_str = format!("{:?}", e);
            if error_str.contains("InvalidChecksum") || error_str.contains("checksum") {
                checksum_error_count += 1;
            }
        }
    }
    
    eprintln!("\n=== Rejet des checksums invalides ===");
    eprintln!("Messages avec checksum invalide testés: {}", lines.len());
    eprintln!("Rejetés correctement: {}", checksum_error_count);
    
    if lines.len() > 0 {
        let rejection_rate = (checksum_error_count as f64 / lines.len() as f64) * 100.0;
        eprintln!("Taux de rejet: {:.2}%", rejection_rate);
        
        // Au moins 90% des checksums invalides doivent être rejetés
        assert!(
            rejection_rate >= 90.0,
