//! Tests avec un grand volume de données SBS (Mode-S/ADS-B)
//! Utilise des fichiers contenant > 10K messages pour tester la robustesse et les performances

use aftn::SbsParser;

/// Test de parsing d'un grand volume de messages SBS
...
fn test_parse_large_sbs_dataset() {
    let dataset_path = "tests/samples/sbs/sbs_large_dataset.txt";
...
#[test]
#[ignore]
fn test_parse_and_validate_large_sbs_dataset() {
    let dataset_path = "tests/samples/sbs/sbs_large_dataset.txt";
    ...
    for (line_num, line) in lines.iter().enumerate() {
        ...
        if let Ok(message) = SbsParser::parse_message(trimmed) {
...
#[test]
#[ignore]
fn test_sbs_parsing_performance() {
    let dataset_path = "tests/samples/sbs/sbs_large_dataset.txt";
    ...
    for line in &lines {
        if SbsParser::parse_message(line.trim()).is_ok() {
            parsed_count += 1;
        }
    }
